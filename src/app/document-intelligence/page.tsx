...
export default function DocumentIntelligencePage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-sans text-slate-100 py-10 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      
      {/* Background Radial Glow Blobs */}
      <div className="absolute top-0 left-1/3 w-[500px] h-[500px] bg-amber-500/10 rounded-full blur-3xl pointer-events-none -translate-y-1/2" />
      <div className="absolute bottom-0 right-1/4 w-[500px] h-[500px] bg-navy/30 rounded-full blur-3xl pointer-events-none translate-y-1/2" />

      <div className="max-w-7xl mx-auto space-y-8 relative z-10">
        
        {/* Header Title Section in Obsidian Theme */}
        <div className="text-center space-y-3.5 max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-500/20 via-amber-500/10 to-amber-500/20 border border-amber-500/30 text-amber-400 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider shadow-lg backdrop-blur-md">
        ...
        {/* 1. Central Hero Drag & Drop Zone in Obsidian Glass */}
        <div className="bg-slate-900/80 border-2 border-dashed border-amber-500/30 hover:border-amber-400 rounded-3xl p-8 sm:p-12 text-center shadow-2xl backdrop-blur-xl transition-all relative overflow-hidden group">
          <input
            ref={fileInputRef}
            type="file"
            className="hidden"
          ...
          <div 
            ...
            className={`space-y-4 cursor-pointer py-2 rounded-2xl transition-colors ${dragOver ? 'bg-amber-500/10' : ''}`}
            onClick={() => fileInputRef.current?.click()}
          >
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center text-slate-950 shadow-xl mx-auto transform group-hover:scale-105 transition-transform">
              <Upload className="w-8 h-8 stroke-[2.5]" />
            </div>

            <div className="space-y-1.5">
              <h3 className="text-lg sm:text-2xl font-black text-white">
                Drop your trade document here, or <span className="text-amber-400 underline underline-offset-4">browse files</span>
              </h3>
            ...
            <div className="flex items-center justify-center gap-6 text-xs font-bold text-slate-400 pt-2 flex-wrap">
              <span className="inline-flex items-center gap-1.5 text-slate-300">
                <Lock className="w-3.5 h-3.5 text-amber-400" /> Private & Secure
              </span>
              <span className="inline-flex items-center gap-1.5 text-slate-300">
                <Sparkles className="w-3.5 h-3.5 text-amber-400" /> Zero API Billing Needed
              </span>
              <span className="inline-flex items-center gap-1.5 text-slate-300">
                <MapPin className="w-3.5 h-3.5 text-amber-400" /> 10 Statutory Document Types
              </span>
            </div>
          </div>
        </div>

        {/* 2. Progress State Banner */}
        {processing && (
          <div className="bg-slate-900/90 border border-amber-500/40 text-white rounded-2xl p-6 shadow-2xl backdrop-blur-xl flex items-center gap-4 animate-fade-in-up">
            <RefreshCw className="w-7 h-7 text-amber-400 animate-spin flex-shrink-0" />
            <div className="space-y-1 flex-1">
              <h4 className="text-sm font-black uppercase tracking-wider text-amber-400">Analyzing Trade Document...</h4>
              <p className="text-xs text-slate-300 font-medium">{processingStep || 'Processing statutory parameters...'}</p>
            </div>
          </div>
        )}

        {/* 3. Categorized Benchmark Selectors */}
        <div className="bg-slate-900/80 border border-white/10 rounded-2xl p-6 shadow-2xl backdrop-blur-xl space-y-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-white/10 pb-3">
            ...
            <div className="flex items-center gap-1.5 bg-slate-950 p-1 rounded-xl text-xs font-bold border border-white/5">
              <button
                type="button"
                onClick={() => setSelectedCategory('all')}
                className={`px-3 py-1 rounded-lg transition-all cursor-pointer ${selectedCategory === 'all' ? 'bg-amber-500 text-slate-950 font-black shadow-md' : 'text-slate-400 hover:text-white'}`}
              >
                All (10)
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg transition-all cursor-pointer ${selectedCategory === 'tax' ? 'bg-amber-500 text-slate-950 font-black shadow-md' : 'text-slate-400 hover:text-white'}`}
              >
                Tax & Reg
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg transition-all cursor-pointer ${selectedCategory === 'trade' ? 'bg-amber-500 text-slate-950 font-black shadow-md' : 'text-slate-400 hover:text-white'}`}
              >
                Trade & Customs
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg transition-all cursor-pointer ${selectedCategory === 'quality' ? 'bg-amber-500 text-slate-950 font-black shadow-md' : 'text-slate-400 hover:text-white'}`}
              >
                Quality & Standards
              </button>
            </div>
                ...
                >
                  <div className="space-y-1">
                    <div className="flex items-center justify-between">
                      ...
                      <FileText className={`w-4 h-4 ${isSelected ? 'text-amber-400' : 'text-slate-500'}`} />
                    </div>
                    <h4 className="text-xs font-black text-white line-clamp-1">
                      {meta.name}
                    </h4>
                    <p className="text-[11px] text-slate-400 line-clamp-2 leading-tight font-medium">
                      {meta.description}
                    </p>
                  </div>

                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-md border w-fit bg-white/5 text-amber-300 border-white/10">
                    {meta.category}
                  </span>
                </button>
              );
        ...
        {activeDossier && !processing && (
          <div className="space-y-6 animate-fade-in-up">
            ...
            {scannedHistory.length > 1 && (
              <div className="bg-slate-900/90 border border-white/10 rounded-2xl p-4 shadow-2xl backdrop-blur-xl flex items-center justify-between gap-4 overflow-x-auto">
                <div className="flex items-center gap-2 text-xs font-bold text-slate-300 flex-shrink-0">
                ...
                <div className="flex items-center gap-2 flex-1 overflow-x-auto py-1">
                  {scannedHistory.map((item) => (
                    ...
                    >
                      ...
                      <span className={`w-2 h-2 rounded-full ${item.scores.overall >= 90 ? 'bg-emerald-400' : 'bg-amber-400'}`} />
                    </button>
                  ))}
                </div>
              </div>
                  ...
                  {crossConsistencyResults.map((audit, idx) => (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <h4 className="text-xs font-black text-white">{audit.ruleName}</h4>
                      </div>
                      <p className="text-xs text-slate-300 leading-relaxed font-medium pl-6">
                        {audit.message}
                      </p>
                      {audit.hindiHint && (
                        <p className="text-[11px] text-amber-400/90 pl-6 mt-1 font-medium">
                          🌐 {audit.hindiHint}
                        </p>
                      )}
                    </div>
